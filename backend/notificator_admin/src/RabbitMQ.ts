...
export class RabbitTelegramRequest {
  protected _topic = "notificator-telegram-send-order-request";
...
> implements RabbitPgRequestInterface {
  ...
  async send(): Promise<RabbitResponseAttributes<RabbitRequestToResponseMap[T]>['data'] | null> {
    return new Promise(async (resolve, reject) => {
      const request = {
        topic: this.topic,
        request_id: this.request_id,
...
async function connectToRabbitMQ(retryCount = 3, delay = 5000) {
  while (retryCount > 0) {
    try {
      ...
      for (const queue of [...REQUEST_PG_QUEUES, ...RESPONSE_PG_QUEUES, "notificator-telegram-send-order-request", "notificator-telegram-send-order-response"]) {
...
const setupConsumer = () => {
  for (const queue of [...RESPONSE_PG_QUEUES, "notificator-telegram-send-order-response"]) {
